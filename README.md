...
All benchmark data and raw outputs are available in the [`agmi/outputs/`](agmi/outputs/) directory:

*   [`creative_scripts.json`](agmi/outputs/creative_scripts.json): Contains all scripts generated by the Creative Engine (Divergence-Convergence), including the intermediate "Tree of Thoughts" concepts and their selection scores.
*   [`baseline_scripts.json`](agmi/outputs/baseline_scripts.json): Contains the control group scripts generated using the standard few-shot prompting approach.
*   [`comparison_report.json`](agmi/outputs/comparison_report.json): The comprehensive dataset containing side-by-side creativity scores (mean, standard deviation) for every product across all 6 evaluation criteria.
